...
import { updatePet } from "../../auth/Redux/actions/PetActions";

function EditPet() {
  const pet = useSelector((state) => state.PetReducer.myPets);
          ...
          <form className="flex flex-col  lg:w-4/5 w-full">
            ...
            <Link to="/Browse">
              <button
                ...
                onClick={(e) =>
                  dispatch(updatePet({ id, Title, Description, Image }))
                }
              >
                Edit Pet
              </button>
